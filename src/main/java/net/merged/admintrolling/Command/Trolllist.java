...
public class Trolllist implements CommandExecutor {
    ...
    public boolean onCommand(@NotNull CommandSender sender, @NotNull Command command, @NotNull String label, @NotNull String[] args) {
        if (sender.isOp()) {
            sender.sendMessage(ColorChat.chat("&b/troll &3<Player> forcechat <Message> &b: Forces a player to say something"));
            ...
            sender.sendMessage(ColorChat.chat("&b/troll &3<Player> breaktroll &b: Makes a user unable to break blocks"));
            ...
            sender.sendMessage(ColorChat.chat("&b/troll &3<Player> trap &b: Traps a player in a bedrock and barrier box"));
            sender.sendMessage(ColorChat.chat("&b/troll &3<Player> fakeban &b: Gives a player a fake ban message with a rickroll link"));
            sender.sendMessage(ColorChat.chat("&b/troll &3<Name> fakejoin &b: Gives everyone on the server a fakejoin message with a specific name"));
            sender.sendMessage(ColorChat.chat("&b/troll &3<Name> fakeleave &b: Gives everyone on the server a fakeleave message with a specific name"));
        }
        return true;
    }
}
