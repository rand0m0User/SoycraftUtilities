...
public class TempBan implements CommandExecutor {
    ...
    public boolean onCommand(@NotNull CommandSender sender, @NotNull Command command, @NotNull String label, @NotNull String[] args) {
        if (sender.isOp()) {
            ...
            String msg = "You have been banned from mc.soyjak.party for the following reason:\n\n"
                    ...
                    + "\n\nYour ban was filed on " + formatTime(LocalDateTime.now())
                    + " and expires on " + formatTime(LocalDateTime.now().plus(d))
                    + "\nwhich is " + d.toDays() + " days and " + ((d.toDays() == 0) ? d.toHours() : d.toHours() % d.toDays()) + " hours" + " from now.\n\n"
                    + "According to our server, your IP is: " + ip
                    + ". The name you were posting with \nwas " + args[0] + ".";
            ...
            if (p == null) {
                //Bukkit.getBanList(Type.PROFILE).addBan(args[0], msg, Date.from(LocalDateTime.now().plus(d).toInstant(ZoneOffset.UTC)), sender.getName()).save();
    ...
    private String formatTime(LocalDateTime t) {
        return t.format(DateTimeFormatter.ofPattern("MMM dd,, yyyy")).replace(",,", "th").replace(" 0", " ").replace("1th", "1st").replace("2th", "2nd").replace("3th", "3rd");
    }
